...
import org.owasp.proxy.ssl.SigningService;

public class BouncyCastleSigner implements SigningService {

	private static final String SIGALG = "SHA1withRSA";

	private static BouncyCastleSigner signerInstance = null;

	private BouncyCastleSigner() {
		// Enforce singleton pattern,
	}

	public static BouncyCastleSigner getInstance() {
		if (signerInstance == null) {
			signerInstance = new BouncyCastleSigner();
		}
	...
	public X509Certificate sign(X500Principal subject, PublicKey pubKey, X500Principal issuer, PublicKey caPubKey,
		...
		try {
			 ...
			 * certificate delivered to the caller is provided by the default security
			 * provider instead of BouncyCastle. If we don't do this trick we might
			 * run into trouble when trying to use the CertPath validator.
			 */
			CertificateFactory certificateFactory = CertificateFactory.getInstance("X.509");
			certificate = (X509Certificate) certificateFactory.generateCertificate(new ByteArrayInputStream(holder
																			.getEncoded()));
		} catch (IOException e) {
