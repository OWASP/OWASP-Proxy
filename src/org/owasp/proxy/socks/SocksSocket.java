...
public class SocksSocket extends Socket {
	...
	/**
	 * Tries to connect to given host and port using default proxy. If no
	 * default proxy specified it throws SocksException with error code
	 * SOCKS_NO_PROXY.
	 * 
	 * @param host
	...
	public SocksSocket(Proxy p, String host, int port) throws SocksException,
		...
		if (p == null)
			throw new SocksException(SocksConstants.SOCKS_NO_PROXY);
		// proxy=p;
		proxy = p.copy();
		remoteHost = host;
		remotePort = port;
	...
	private void doDirect() throws SocksException {
		try {
			...
			throw new SocksException(SocksConstants.SOCKS_DIRECT_FAILED,
