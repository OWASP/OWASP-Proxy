...
public class Socks5Proxy extends Proxy implements Cloneable {
	 ...
	 * requests, this option controls whether the hostnames should be send to the
	...
	public Authentication getAuthenticationMethod(int methodId) {
		return authMethods.get(new Integer(methodId));
	}
	...
	protected void startSession() throws SocksException {
		...
		try {
			...
			if (selectedMethod == 0xFF) { // No method selected
				ps.close();
				throw (new SocksException(SocksConstants.SOCKS_AUTH_NOT_SUPPORTED));
			}
			...
			throw (new SocksException(SocksConstants.SOCKS_PROXY_NO_CONNECT));
