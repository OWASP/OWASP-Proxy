...
import org.owasp.webscarab.io.FixedLengthInputStream;

/**
 * The Message class is the base class for the HTTP Request and Response classes.
 * 
 ...
 * </ul>
 * 
 * You can use these methods to examine the raw bytes that were set, e.g. by reading from a network connection.
 * WebScarab will also use these methods when writing the 
 * At the next level, there are two sets of convenience methods, which split the message into
 * &quot;header&quot; and &quot;content&quot;, at the first CRLFCRLF sequence found.
 * <ul>
 * <li> getHeader() </li>
 ...
 * </ul>
 * 
 * Getting more convenient, there are methods that parse the &quot;header&quot; into lines and 
 ...
 * </ul>
 * 
 * Finally, there are two methods that operate on the content, processing it according to the Content-Encoding
...
public class Message {
	...
	public byte[] getContent() throws MessageFormatException {
		...
		if (sep < message.length - separator.length) {
			...
			System.arraycopy(message, sep + separator.length, content, 0, content.length);
			return content;
		}
		return null;
	}
	...
	protected String[] getHeaderLines(byte[] separator) throws MessageFormatException {
		...
		try {
			...
			if (start < header.length)
				lines.add(new String(header, start, header.length - start, "ASCII"));
	 ...
	 * @throws MessageFormatException
	 */
	protected void setHeaderLines(String[] lines, byte[] separator) throws MessageFormatException {
		try {
	 ...
	 * @throws MessageFormatException
	 */
	public void setHeader(String name, String value) throws MessageFormatException {
		NamedValue[] headers = getHeaders();
		if (headers != null && headers.length != 0) {
			for (int i=0;i<headers.length; i++) 
				if (name.equalsIgnoreCase(headers[i].getName())) {
					headers[i] = new NamedValue(name, headers[i].getSeparator(), value);
					setHeaders(headers);
					return;
				}
		}
	 ...
	 * @throws MessageFormatException
	 */
	public void addHeader(String name, String value) throws MessageFormatException {
	 ...
	 * @throws MessageFormatException
	 */
	private void addHeader(NamedValue[] headers, String name, String value) throws MessageFormatException {
		if (headers == null) {
		...
		for (int i = 0; i < headers.length; i++)
			if (name.equalsIgnoreCase(headers[i].getName())) {
				...
				if (i < headers.length - 1)
					System.arraycopy(headers, i + 1, nh, i, headers.length - i - 1);
				setHeaders(nh);
				return ret;
			}
		return null;
	 ...
	 * @throws MessageFormatException
	 */
	private byte[] decode(byte[] content, String codings) throws MessageFormatException {
		...
		try {
			...
			for (int i=0; i<algos.length; i++) {
				if ("chunked".equalsIgnoreCase(algos[i])) {
					...
					throw new MessageFormatException("Unsupported coding : " + algos[i]);
			}
	 ...
	 * @throws MessageFormatException
	 */
	private byte[] encode(byte[] content, String codings) throws MessageFormatException {
		if (codings == null || codings.trim().equals(""))
			return content;
		try {
			String[] algos = codings.split("[ \t]*,[ \t]*");
	...
	public byte[] getEntity() throws MessageFormatException {
		byte[] transportDecoded = decode(getContent(), getHeader("Transport-Encoding"));
		byte[] contentDecoded = decode(transportDecoded, getHeader("Content-Encoding"));
	...
	public void setEntity(byte[] decoded) throws MessageFormatException {
		...
		byte[] transportEncoded = encode(contentEncoded, getHeader("Transport-Encoding"));
	...
	protected void setStartParts(String[] parts) throws MessageFormatException {
		if (parts == null || parts.length == 0) {
			...
			StringBuilder b = new StringBuilder(parts[0] == null ? "" : parts[0]);
	...
	public static boolean flushContent(Message message, InputStream in, OutputStream out) throws MessageFormatException, IOException {
